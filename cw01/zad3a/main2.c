...
void load_functions(){
    void *uchwyt=dlopen("./libw_count.so", RTLD_LAZY);
    if(!uchwyt){printf("blad otw biblioteki\n"); exit(-1);}

    make_memory_block = dlsym(uchwyt, "make_memory_block");
    if(dlerror()!=NULL){printf("blad z make_memory_block\n"); exit(-1);}

    write_to_block = dlsym(uchwyt, "write_to_block");
    if(dlerror()!=NULL){printf("blad z write_to_block\n"); exit(-1);}

    get_data = dlsym(uchwyt, "get_data");
    if(dlerror()!=NULL){printf("blad z get_data\n"); exit(-1);}

    delete_data = dlsym(uchwyt, "delete_data");
    if(dlerror()!=NULL){printf("blad z delete_data\n"); exit(-1);}

    free_pointers = dlsym(uchwyt, "free_pointers");
    if(dlerror()!=NULL){printf("blad z free_pointers\n"); exit(-1);}
}
